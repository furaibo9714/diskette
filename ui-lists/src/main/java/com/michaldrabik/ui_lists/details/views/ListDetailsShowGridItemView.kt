...
class ListDetailsShowGridItemView : ListDetailsItemView {
  ...
  override fun bind(item: ListDetailsItem) {
    ...
    with(binding) {
      ...
      listDetailsGridTitle.text = if (item.translation?.title.isNullOrBlank()) show.title else item.translation?.title
  ...
  private fun clear() {
    with(binding) {
      ...
      Glide.with(this@ListDetailsShowGridItemView).clear(listDetailsGridImage)
    }
  }
}
