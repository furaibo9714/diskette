...
class ListDetailsShowCompactItemView : ListDetailsItemView {
  ...
  override fun bind(item: ListDetailsItem) {
    ...
    with(binding) {
      ...
      listDetailsCompactTitle.text =
        if (item.translation?.title.isNullOrBlank()) show.title else item.translation?.title
  ...
  private fun clear() {
    with(binding) {
      ...
      Glide.with(this@ListDetailsShowCompactItemView).clear(listDetailsCompactImage)
    }
  }
}
