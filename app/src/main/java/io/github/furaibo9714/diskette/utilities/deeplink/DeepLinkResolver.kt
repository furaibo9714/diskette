...
class DeepLinkResolver @Inject constructor() {
  ...
  fun resolveDestination(
    ...
    try {
      ...
      val navBundle = bundleOf(NavigationArgs.ARG_SHOW_ID to show.mediaId)
      ...
      Logger.record(error, "DeepLinkResolver::resolveDestination(show:${show.mediaId})")
    }
  }
  ...
  fun resolveDestination(
    ...
    try {
      ...
      val navBundle = bundleOf(NavigationArgs.ARG_MOVIE_ID to movie.mediaId)
      val actionId = when (navController.currentDestination?.id) {
      ...
      Logger.record(error, "DeepLinkResolver::resolveDestination(movie:${movie.mediaId})")
    }
  }
