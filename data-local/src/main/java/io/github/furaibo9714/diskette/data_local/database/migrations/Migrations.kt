...
class Migrations(
  ...
  private val migration42 = object : Migration(41, 42) {
    override fun migrate(database: SupportSQLiteDatabase) {
      with(database) {
        execSQL(
          "CREATE TABLE IF NOT EXISTS `floppy_sync_queue` (" +
            "`id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, " +
  ...
  private val migration45 = object : Migration(44, 45) {
    override fun migrate(database: SupportSQLiteDatabase) {
      with(database) {
        execSQL("ALTER TABLE custom_lists ADD COLUMN id_floppy INTEGER")
        execSQL("ALTER TABLE floppy_sync_queue ADD COLUMN list_id INTEGER")
      }
    }
  }
  ...
  private val migration46 = object : Migration(45, 46) {
    override fun migrate(database: SupportSQLiteDatabase) {
      with(database) {
        execSQL("DROP TABLE IF EXISTS sync_trakt_log")
      }
    }
  }
  ...
  private val migration48 = object : Migration(47, 48) {
    override fun migrate(database: SupportSQLiteDatabase) {
      database.execSQL("ALTER TABLE shows ADD COLUMN runtime_max INTEGER NOT NULL DEFAULT -1")
    }
  }
