...
interface PeopleDao : BaseDao<Person>, PeopleLocalDataSource {
  ...
  @Query(
    ...
    WHERE people_shows_movies.show_media_id = :showMediaId
    """
  )
  override suspend fun getAllForShow(showMediaId: String): List<Person>
  ...
  @Query(
    ...
    FROM people
    INNER JOIN people_shows_movies ON people_shows_movies.id_tmdb_person = people.id_tmdb
    WHERE people_shows_movies.movie_media_id = :movieMediaId
    """
  )
  override suspend fun getAllForMovie(movieMediaId: String): List<Person>
