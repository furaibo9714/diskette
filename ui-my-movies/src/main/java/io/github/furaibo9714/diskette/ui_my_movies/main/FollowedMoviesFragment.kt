...
import io.github.furaibo9714.diskette.ui_my_movies.databinding.FragmentFollowedMoviesBinding
...
class FollowedMoviesFragment :
  ...
  fun openMovieDetails(movie: Movie) {
    ...
    binding.followedMoviesRoot
      .fadeOut(150) {
        val bundle = Bundle().apply { putString(ARG_MOVIE_ID, movie.mediaId.key) }
  ...
  fun openMovieMenu(movie: Movie) {
    ...
    val bundle = ContextMenuBottomSheet.createBundle(movie.ids.media)
  ...
  private fun render(uiState: FollowedMoviesUiState) {
    uiState.isSyncing?.let {
      binding.followedMoviesSearchView.setSyncProgress(it, uiState.syncPhaseState?.format(requireContext()))
