...
import io.github.furaibo9714.diskette.ui_show.databinding.ViewQuickSetupBinding
...
class QuickSetupView : FrameLayout {
  ...
  private fun onItemChecked(
    ...
    val item = items
      ...
      .first { it.episode.ids.media == episode.ids.media }
      .copy(isChecked = !isChecked)

    items.findReplace(item) { it.episode.ids.media == episode.ids.media && !it.isHeader }
