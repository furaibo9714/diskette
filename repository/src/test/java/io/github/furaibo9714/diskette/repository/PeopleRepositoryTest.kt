...
class PeopleRepositoryTest : BaseMockTest() {
  ...
  @RelaxedMockK lateinit var mediaRemoteSource: MediaRemoteDataSource
  ...
  override fun setUp() {
    ...
    coEvery { cloud.media } returns mediaRemoteSource
  }
  ...
  fun `Should return local data for shows properly`() =
    runBlocking {
      ...
      SUT.loadAllForShow(Ids.EMPTY.copy(media = MediaId.tmdb(11)))

      coVerifyOrder {
        peopleShowsMoviesDao.getTimestampForShow("tmdb:11")
        peopleDao.getAllForShow("tmdb:11")
      }
  ...
  fun `Should return remote data for shows properly`() =
    runBlocking {
      ...
      SUT.loadAllForShow(Ids.EMPTY.copy(media = MediaId.tmdb(11), tmdb = IdTmdb(12)))
      ...
      coVerifyOrder {
        ...
        peopleShowsMoviesDao.insertForShow(any(), "tmdb:11")
      }
    }
  ...
  fun `Should return local data for movies properly`() =
    runBlocking {
      ...
      SUT.loadAllForMovie(Ids.EMPTY.copy(media = MediaId.tmdb(11)))

      coVerifyOrder {
        peopleShowsMoviesDao.getTimestampForMovie("tmdb:11")
        peopleDao.getAllForMovie("tmdb:11")
      }
  ...
  fun `Should return remote data for movies properly`() =
    runBlocking {
      ...
      SUT.loadAllForMovie(Ids.EMPTY.copy(media = MediaId.tmdb(11), tmdb = IdTmdb(12)))
      ...
      coVerifyOrder {
        ...
        peopleDao.upsert(any())
        peopleShowsMoviesDao.insertForMovie(any(), "tmdb:11")
      }
    }
  ...
  fun `Should return shows items with image in the first place`() =
    runBlocking {
      ...
      val result = SUT.loadAllForShow(Ids.EMPTY.copy(media = MediaId.tmdb(11)))
  ...
  fun `Should return movies items with image in the first place`() =
    runBlocking {
      ...
      val result = SUT.loadAllForMovie(Ids.EMPTY.copy(media = MediaId.tmdb(11)))
      assertThat(result[Department.ACTING]!!.first().imagePath).isNotNull()
  ...
  fun `Should return locally cached credits if cache is valid`() =
    runBlocking {
      ...
      coVerify(exactly = 0) { mediaRemoteSource.fetchPersonShowsCredits(any(), any()) }
      coVerify(exactly = 0) { mediaRemoteSource.fetchPersonMoviesCredits(any(), any()) }
    }
  ...
  fun `Should return remote credits if cache is invalid`() =
    runBlocking {
      ...
      coEvery { peopleCreditsDao.getTimestampForPerson(any()) } returns null
      coEvery { mediaRemoteSource.fetchPersonShowsCredits(any(), any()) } returns listOf(creditsShows)
      coEvery { mediaRemoteSource.fetchPersonMoviesCredits(any(), any()) } returns listOf(creditsMovies)
