...
class StatisticsMoviesViewModelTest : BaseMockTest() {
  ...
  internal fun `Should load statistics properly`() =
    runTest {
      val movies = listOf(
        Movie.EMPTY.copy(ids = Ids.EMPTY.copy(trakt = MediaId.parse(1)), runtime = 1, genres = listOf("war", "drama")),
        Movie.EMPTY.copy(ids = Ids.EMPTY.copy(trakt = MediaId.parse(2)), runtime = 2, genres = listOf("war", "animation")),
        Movie.EMPTY.copy(ids = Ids.EMPTY.copy(trakt = MediaId.parse(3)), runtime = 3, genres = listOf("war", "animation")),
      )
